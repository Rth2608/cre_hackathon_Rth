...
var configSchema = exports_external.object({
  authorizedKey: exports_external.string().regex(/^0x[a-fA-F0-9]{40}$/),
  embeddingUrl: exports_external.string().regex(/^https?:\/\/\S+$/),
  embeddingModel: exports_external.string().min(1),
  policyVersion: exports_external.string().min(1),
  passMinEmbeddingDims: exports_external.number().int().min(1).max(32768)
});
var inputSchema = exports_external.object({
  question: exports_external.string().min(1),
  description: exports_external.string().min(1),
  sourceUrls: exports_external.array(exports_external.string().regex(/^https?:\/\/\S+$/)).default([]),
  resolutionCriteria: exports_external.string().min(1),
  submitterAddress: exports_external.string().regex(/^0x[a-fA-F0-9]{40}$/)
});
var requestSchema = exports_external.object({
  requestId: exports_external.string().min(1),
  input: inputSchema,
  node: exports_external.object({
    nodeId: exports_external.string().min(1),
    modelFamily: exports_external.string().min(1),
    modelName: exports_external.string().min(1),
    operatorAddress: exports_external.string().regex(/^0x[a-fA-F0-9]{40}$/)
  }).optional()
});
function normalizeUrls(urls) {
  return [...urls].map((value2) => value2.trim()).filter(Boolean).sort((a, b) => a.localeCompare(b));
}
function buildCanonicalText(payload) {
...
function computeConfidence(embeddingDims) {
  if (embeddingDims <= 0) {
    return 0;
  }
...
async function onHttpTrigger(runtime2, payload) {
  const config = configSchema.parse(runtime2.config);
  if (config.embeddingModel === "openai-embedding-model-id") {
    throw new Error("embedding_model_not_configured: set config.embeddingModel to a real OpenAI embedding model your team can access");
  }
  const request = requestSchema.parse(decodeJson(payload.input));
  const canonicalText = buildCanonicalText(request);
  const confidentialClient = new ClientCapability2;
  const response = confidentialClient.sendRequest(runtime2, {
    vaultDonSecrets: [
      {
        key: "llmApiKey"
      }
  ...
  const generatedAt = runtime2.now().toISOString();
  return {
    report: {
      verdict,
      confidence,
      rationale: verdict === "PASS" ? "Confidential embedding generation succeeded and minimum embedding-dimension policy passed." : "Confidential embedding generation succeeded but minimum embedding-dimension policy failed.",
...
function initWorkflow(config) {
  ...
  return [cre.handler(trigger, (runtime2, payload) => onHttpTrigger(runtime2, payload))];
}
async function main() {
  const runner = await Runner.newRunner({
    configParser: (configBytes) => decodeJson(configBytes)
  });
